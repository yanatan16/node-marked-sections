...
function mergeOptions(opts) {
	...
	options.hierarchy = options.heirarchy !== undefined ? options.heirarchy : options.hierarchy;

	marked.setOptions(opts);
}

function openTag(n) {
	return htmlTag(_.repeat('<section>', options.hierarchy ? (options.deep ? options.levels : n) : 1).join(''));
}
...
function middleTag(a, b) {
	...
	if (options.hierarchy && options.deep) {
