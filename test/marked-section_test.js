...
tests.testHeaderHeirarchyDeep = function (test) {
	var text = '# head1\ntest\n\n## head2\n- l1\n- l2\n\n# head12';
	var output = mdsec.parse(text, {levels: 2, heirarchy: true, deep: true}).replace(/[\n\t]/g, '');
...
tests.testHeaderHeirarchy = function (test) {
	var text = '# head1\ntest\n\n## head2\n- l1\n- l2\n\n# head12';
	var output = mdsec.parse(text, {levels: 2, heirarchy: true, deep: false}).replace(/[\n\t]/g, '');

	test.equal('<section><h1>head1</h1><p>test</p><section><h2>head2</h2><ul><li>l1</li><li>l2</li></ul></section></section><section><h1>head12</h1></section>', output);
	test.done();
}
